# -*- coding: utf-8 -*-
"""
Created on Thu Nov 02 11:08:57 2017
...
class GetData():
    def Data(self):
        ...
        cursor=db.cursor()
        Content=[]
        sql='select day,authorscore,categoryscore,titlescore,summaryscore,titlepolarity,logscan from spiderdb.paper'              
        cursor.execute(sql)
...
for train_ratio in [0.9,0.6,0.3]:
    ...
    for i in range(5):
        ...
        min_max_scaler = preprocessing.MinMaxScaler(feature_range=(10, 15))  #1-3 10-12
        y_train=np.array(y_train).reshape(-1,1)
        y_train = min_max_scaler.fit_transform(y_train)
        y_test=np.array(y_test).reshape(-1,1)
        y_test = min_max_scaler.transform((y_test))
        #print "\n y train",y_train[0:10]
        #y_train = preprocessing.normalize(y_train, norm='l2').T
        #y_test = preprocessing.normalize(y_test, norm='l2').T
        print "x_train shape",x_train[0]
        # 建立模型
        model = Sequential()
        model.add(Embedding(10000,128))
        model.add(Dropout(0.25)) 
        model.add(LSTM(128,dropout_U=0.25)) #,W_regularizer=regularizers.l2(10e-3)
        model.add(Dense(1))
